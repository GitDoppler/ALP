...
; code starts here
PUSH AX
PUSH DX ; store the value to not lossing it
...
;Nu merge si cel mai probabil nu este corect. Imi cer scuze.
PUSH AX
PUSH BX
PUSH SI
PUSH DI
...
MOV SI, 2     ;incepem de la al doilea index pentru ca buffer[1] este lungimea input-ului
MOV DI, 0     ;initializam index-ul pt VECTOR
MOV CL, buffer[1]
INC CL     
MOV CH, 0  ;ne asiguram ca high byte-ul lui CX este gol

next_entry:
    MOV AL, buffer[SI]  ;obtinem byte-ul din buffer
    CMP AL, ',' ;verificam daca este o virgula
    JZ update_index ;daca este virgula, dam skip la urmatorul byte
    SUB AL, '0' ;convertim din ASCII in digit
    MOV VECTOR[DI], AL ;dam store la digit in VECTOR
    INC DI  ;mergem la urmatorul index in VECTOR
update_index:
    INC SI  ;mergem la urmatorul byte in buffer
    CMP SI, CX  ;comparam cu lungimea input-ului
    JBE next_entry  ;daca nu am ajuns la final, continuam
